...
public class UIController : Singleton<UIController> 
{
    ...
    void ResetAllUI()
    {
        foreach (CanvasGroup panel in uiPanels.Values)
            panel.gameObject.SetActive(false); //turn off all the panels
    ...
    {
        if (panel == currentPanel) //on current panel
            return;
        //otherwise
        if (currentPanel)
            FadeOut(currentPanel); //turn off old panel
        currentPanel = panel; //set new panel
        if (panel)
            FadeIn(panel); //turn on new panel
    }
